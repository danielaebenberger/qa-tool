...
import {
  ...
  type Capability,
} from '../../scripts/generate-capabilities';
...
describe('parseFrontmatter', () => {
  ...
  it('parses YAML folded-scalar descriptions with > indicator', () => {
    const content = [
      '---',
      'name: qa-ac-validate',
      'description: >',
      ...
      '    REFINEMENT: early conflict detection',
      '    VALIDATION: full coverage check',
      'kind: skill',
    ...
    expect(fm).toEqual({
      name: 'qa-ac-validate',
      description:
        'Pillar A — Acceptance Criteria Validation. Operates in two modes: REFINEMENT: early conflict detection VALIDATION: full coverage check',
      kind: 'skill',
      pillar: 'feature-validation',
      version: '1.0',
    });
